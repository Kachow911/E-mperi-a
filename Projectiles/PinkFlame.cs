    ...
    public class PinkFlame : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] > 12f)  //this defines where the flames starts
            {
                ...
                {
                    int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 58, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, mod.DustType("PinkFire"), default(Color), 3.75f);   //this defines the flames dust and color, change DustID to wat dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
                    Main.dust[dust].noGravity = true; //this make so the dust has no gravity
                    Main.dust[dust].velocity *= 2.5f;
                    int dust2 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 58, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, mod.DustType("PinkFire"), default(Color), 1.5f); //this defines the flames dust and color parcticles, like when they fall thru ground, change DustID to wat dust you want from Terraria
                }
            }
            else
            {
        ...
        {
            target.AddBuff(mod.BuffType("OnPinked"), 80);   //this make so when the projectile/flame hit a npc, gives it the buff  onfire , 80 = 3 seconds
        }
